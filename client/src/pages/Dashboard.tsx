...
export default function Dashboard() {
  ...
  return (
    <section className="px-4 py-6 max-w-md mx-auto">
      <h2 className="heading text-xl font-bold mb-4 border-b-2 border-black pb-2">Dashboard</h2>
      
      <div className="grid grid-cols-2 gap-4 mb-6">
        {dashboardCards.map((card, index) => (
          <div 
            key={index} 
            className="neu-card-colorful p-4 flex flex-col"
          ...
          >
            ...
            <div className="mt-2 flex items-center bg-white px-2 py-1 self-start">
              <i className={`fa-solid ${card.icon} text-xs mr-1 text-black`}></i>
      ...
      <div className="neu-card p-4 mb-6">
        <h3 className="title text-lg font-semibold mb-4 border-b-2 border-black pb-2">Proyek Aktif</h3>
        {activeProjects.length > 0 ? (
              ...
              return (
                <div key={project.id} className="border-b-2 border-black pb-3">
                  <div className="flex justify-between">
                    ...
                    <span className="neu-badge">Aktif</span>
                  </div>
                  <p className="text-sm mt-1 font-medium">{project.address}</p>
                  <div className="flex justify-between mt-2">
                    <span className="text-sm font-bold bg-secondary text-white px-2">{project.workers.length} Pekerja</span>
                    <span className="text-sm font-bold bg-primary text-white px-2">{formatCurrency(totalSalary)}</span>
                  </div>
                </div>
              );
            })}
          ...
          <div className="text-center py-8 border-2 border-dashed border-black">
            ...
            <Link href="/custom">
              <div className="mt-4 inline-block py-2 px-4 bg-primary text-white border-2 border-black font-bold shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                + Tambah Proyek Baru
              </div>
            </Link>
          </div>
